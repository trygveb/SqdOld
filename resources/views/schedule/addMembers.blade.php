...
<h1>{{__('Add Members')}} NOT USED</h1>
 <div class="container">
     ...
     <br>
      <form action="{{ route('schedule.addMember')}}" method="POST">
          {{ csrf_field() }}
         <input type="hidden" name="scheduleId" value="{{$schedule->id}}">
         <fieldset>
             <legend>{{__('Check members to be added to schedule')}}: <span style="white-space: nowrap;">{{$schedule->name}}</span></legend>
            <br>
      ...
      @else
         {{__('All registered users are already connected to')}} <span style="white-space: nowrap;">{{$schedule->name}}</span>.
      @endif
            </div>
            <br>
            <x-submit-button
